    ...
    <div className="min-h-screen bg-bg-base font-inter flex flex-col md:flex-row overflow-hidden selection:bg-accent/30">
      ...
      >
        <div className="max-w-md w-full mx-auto">
          ...
          {/* ROLE SELECTOR */}
          <div className="flex gap-3 mb-8 p-1 bg-white/5 rounded-2xl border border-white/5">
            <button
              type="button"
              onClick={() => setRole('user')}
              className={`flex-1 flex flex-col items-center gap-2 py-4 rounded-xl transition-all ${
                role === 'user' 
                ? 'bg-accent text-white shadow-lg shadow-accent/20 ring-1 ring-white/20' 
                : 'text-text-light/40 hover:bg-white/5 hover:text-white'
              }`}
            >
              <User size={20} />
              <span className="text-[10px] font-bold uppercase tracking-widest">Atlet</span>
            </button>
            <button
              type="button"
            ...
            >
              <Shield size={20} />
              <span className="text-[10px] font-bold uppercase tracking-widest">Pelatih</span>
            </button>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-6">
            ...
            >
              {isRegistering ? 'Mendaftarkan...' : `Daftar Sebagai ${role === 'user' ? 'Atlet' : 'Pelatih'}`} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </button>
